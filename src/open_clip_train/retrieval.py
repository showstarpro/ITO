...
def collate_fn(batch):
    ...
    texts = [item[1] for item in batch]  # 保持为列表
    
    return images, texts

def encode_docci_dataset(clip, data, device, batch_size = 16):
    ...
    with torch.no_grad():
        ...
        if "ms-coco" in data:
            ...
            dataset = data['docci']
        if "docci" in data:
            dataloader = dutils.DataLoader(dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn)
        else:
        ...
        for images, text in dataloader:
            ...
            for i in range(batch_size):
                txt = text[i]
                txt = txt.to(device)
                captions_per_image, _ = txt.shape
                ...
                image_index += 1
                text_encodings.append(clip.encode_text(txt)[0])
            # text = torch.flatten(text, start_dim=0, end_dim=1)
                                        
            image_encodings.append(clip.encode_image(images)[0])
            # text_encodings.append(clip.encode_text(text)[0])
...
def recall_at_k_docci(clip, dataset: dutils.Dataset, device, k_vals: List[int], batch_size: int):
    print("Encoding all data...")
    # image_encodings, text_encodings, text_to_image_map, image_to_text_map = encode_dataset(clip, dataset, device, batch_size=batch_size)
    image_encodings, text_encodings, text_to_image_map, image_to_text_map = encode_docci_dataset(clip, dataset, device, batch_size=batch_size)
    ...
    captions_per_image = []
    for i in range(len(image_to_text_map)):
        captions_per_image.append(image_to_text_map[i].shape[0])
    ...
    for k in k_vals:
        ...
        for i in range(num_im):
            gt_indices = image_to_text_map[i]
            contains_index = torch.eq(topk[i].unsqueeze(0), gt_indices.unsqueeze(1)).any()
            correct[i] = contains_index
